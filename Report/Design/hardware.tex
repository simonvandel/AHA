...
As specified in \cref{sec:architecture}, rules are to be stored on the computational devices controlling the sensors and actuators. As specified in \cref{\sinote{afsnit hvor vi snakker om regler}}, a rule has the size of x \sinote{Indsæt rigtigt tal når det er klart} bytes. The hardware platform should be able to store x \sinote{indsæt rigtigt tal} rules at any given time.
