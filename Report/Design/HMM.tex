...
To give an intuitive understanding of the hidden states in the HMM used in this example, $x_0$ represents the user drinking coffee in the room. $x_1$ represents the user walking into the living room with the cup of coffee, and putting it on the table. $x_2$ is when the user realises an urgen to go to the restroom to urinate, so the user walks into the hallway. \Cref{fig:pred_example} illustrates this in a trellis diagram.
...
First, we use the Viterbi algorithm to find the most likely sequence of hidden states $x_0,x_1,x_2$ that generated the observations $O_0,O_1,O_2$. Observation $O_3$ is generated by $x_3$. We can find this hidden state by finding the most likely transition from $x_2$. That is we compute $a_{2, j}$ for all hidden states $j$, and find the one with the maximum probability. The observation $O_3$ can now be found by finding the most likely emission from $x_3$. This is done by computing $b_2(k)$ for all emission states $k$, and find the one with the maximum probability.
