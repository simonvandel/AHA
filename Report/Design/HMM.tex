...
\begin{tikzpicture}
  ...
  \draw [->, to path={-| (\tikztotarget)}] (ht) edge[out=0,in=180] (htf1);
\end{tikzpicture}
\caption[Absolute and relative time trellis diagram for a HMM]{An absolute and relative time trellis diagram for a 1st order HMM. The x_{1}, x_{2} are absolute times where x_{t-1}, x_{t} are relative, since the time depend on t \label{fig:4thMarkovModel}
\end{figure}
