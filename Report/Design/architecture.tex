...
\end{tikzpicture}
\caption{Illustration of a monolithic system. The central processing unit monitors the sensors and, at the same time, analyses the monitored sensor data.}\label{fig:monolithic_system}
\end{figure}
...
\end{tikzpicture}
\caption{Illustration of a distributed system. In this example, two controllers monitor their respective sensors and analyse the monitored data. The knowledge from the analysis is shared among different controllers in the system.}\label{fig:distributed_system}
\end{figure}
...
\end{tikzpicture}
\caption{Illustration of a separated system. The sensor and actuator subsystem each consists of one controller, controlling one sensor and actuator respectively.}\label{fig:separated_subsystems}
\end{figure}
...
The difference between scheme 3 and 2 is that the sensor and actuator subsystems have been merged. See \cref{fig:separated_subsystems_scheme3}. This means that sensors and actuators can be connected to the same controller\ranote{gios correction here seems wiered?}, reducing the latency for communicating sensor data between actuators and sensors.
