...
\item \textbf{Amount of communication} The less time spent communicating, the more time can be spent on other activities. If too much time is spent on communication, it could mean that some deadlines are not met
...
\begin{itemize}
\item \textbf{Number of critical communication paths} x
\item \textbf{Memory consumption on actuator and sensor subsystems} x
\item \textbf{Amount of communication} x
\item \textbf{Inter-dependency complexity} x
\item \textbf{Memory consumption on analysis subsystem} x
\end{itemize}
