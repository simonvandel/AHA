...
One possibility is to have one central processing unit that receives data from all available sensors, sanitises and analyses it, decides which, if any, action to perform, and performs it via its actuators. This is illustrated in \cref{fig:monolithic_system}. The system could be made in two ways.
...
If the setup is managed by the user, it is vulnerable to user mistakes, which could render the whole system useless. For example, consider the case where a PIR sensor is connected to the system, but the system believes the connected sensor to be a thermometer. The data received from the PIR sensor will most likely not make any sense, so the system will be based on a wrong model of the world.
...
To form the complete system, some communication must take place between the aforementioned subsystems. There are different possible designs.
...
Given the above architectural designs, the monolithic design is too restrictive in its extensibility. The distributed design allows for greater extensibility, but at the cost of the need for a complex synchronisation of knowledge between subsystems. The seperated design allows for extensibility of sensors and actuators without complex synchronisation. Therefore the seperated design seems the most sensible choice.
...
\item \textbf{Memory consumption on actuator and sensor subsystems.} As the actuator and sensor subsystems should be energy efficient, the memory on-board the controllers is not very spacious
...
\item \textbf{Memory consumption on analysis subsystem.} As the analysis subsystem is not required to be low powered, it is not overly important to consider this
\end{enumerate}
...
\begin{tabular}{m{7em} m{7em} m{8em} m{8em} m{8em} m{8em}}
\toprule
...
\end{tabular}
\end{adjustbox}
\caption{Evaluation table of communication schemes. S means sensor subsystem. Ac means actuator subsystem. A means analysis subsystem.}\label{tab:scheme_choice}
\end{table}

Looking at \cref{tab:scheme_choice}, the following ranking of the schemes can be produced.

\begin{itemize}
\item Scheme 3 is best, however only if rules can be stored on sensor/actuator subsystem
\item Scheme 1 is second best. Best if rules can not be stored on sensor/actuator subsystem
\item Scheme 4 is third best
\item Scheme 5 is fourth best
\item Scheme 2 is fifth best
\end{itemize}