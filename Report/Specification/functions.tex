...
In the table below, \cref{table:functionlist}, each function is described as an action the system can take or information the system have acess to. Each function also have two attributes. \textit{Complexity} is an aproximation\ranote{i virkeligheden er det vel et gæt, men kan vi skrive det? Måske bare et bedre ord end aprox.?} of how complex developing the function will be. \textit{Type} describes what the function does in the system\cite{OOAD}.\ranote{skal vi beskrive dem alle ,eller er det nok at linke til bogen?}
\begin{table}[hbtp]
\centering
\begin{tabular}{lcr}
\toprule
...
The \enquote{User enter room} function is of Type \enquote{Update} because its activated by a change in the problem domain and presents a model change in the system. Because this function requires the system i got nothing why is this function medium complexity?\ranote{IMPORTANT TEMP TEXT, erm hvorfor er den medium komplesitet}
