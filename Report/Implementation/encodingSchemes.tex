...
The primary concerns of choosing an encoding for our communication subsystem, are minimising the size of the packages communicated and ensuring flexibility. The system must be maintainable, since changes in the encoding can occur depending the future improvements, furthermore the resposiveness of the systemis directly dependt on size of the packages\ahnote{referes til sektion test af xbee moduler}. An implementation of a simple domain specific encoding scheme is described below. This will cover schemes to and from the sensor/actuator subsystem and the artificial intelligence subsystem. Note that the encodings assume little endianness.

The module itself is rather specific for the current sensor, this could impact the maintainability, however this was chosen because the of the unreliability of Xbee modules. It was found through testing that certain bytes are not transfered, the current encoding module avoids these complications in way that a general implementation could not. However the system satifies maintainability since the encoding subsystem is a module that can easily be switched with another encoding module if neccesary.
