...
As discussed earlier, the communication in the system will be done using XBee modules, using the ZigBee protocol\kanote{this is the first time we mention xbee.. awkwaaaard}. In this section, the specific implementation in this system will be discussed, along with the impact this choice could have on the real-time system\kanote{do we mean sensor/actuator subsystem? (which is a fantastic name btw)} requirements.
...
The worst case time of a packet arriving and getting\kanote{getting?} before just getting dropped is given by $\text{WORST}$ assuming the ACK arrived 0.1ms before ACK wait timer ran out.
\begin{equation*}
