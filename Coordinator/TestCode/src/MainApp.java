...
public class MainApp {
	...
	public static void main(String[] args) {
		...
		System.out.println("myDevice init done.");
		if (myDevice.isOpen())
			System.out.println("Device open");
		else {
			try {
				myDevice.open();
				System.out.println("Device was closed, now opened");
		...
		/*
		 * 
		 * while (true) { // wait 1000 miliseconds for data XBeeMessage xBMsg =
		 * RecieveData(myDevice); // if data recieved succesfully if (xBMsg !=
		 * null) { try { System.out.format("From %s >> %s | %s%n",
		 * xBMsg.getDevice() .get64BitAddress(), // source addr
		 * HexUtils.prettyHexString(HexUtils
		 * .byteArrayToHexString(xBMsg.getData())), // data // in // hex new
		 * String(xBMsg.getData())); // data in text // init return string
		 * String toSend = "Ok";
		 * myDevice.setDestinationAddress(xBMsg.getDevice() .get64BitAddress());
		 * // set DL/DH (target addr) to // source of msg
		 * BroadcastSend(toSend.getBytes(), myDevice); } catch (XBeeException e)
		 * { e.printStackTrace(); } finally { if(myDevice.isOpen())
		 * myDevice.close(); try {
	...
	public static void TargetSend(byte[] dataToSend, XBeeDevice myDevice) {
		...
		try {
			...
			RemoteXBeeDevice remoteDevice = xbeeNetwork.getDevice(myDevice
